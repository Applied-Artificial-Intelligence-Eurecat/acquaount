...
valori_irr_wheat=rep("S",each=length(clim))
...
valori_irr_maize=rep("S",each=length(clim))
...
valori_irr_rice=rep("G",each=length(clim))
...
valori_irr_barley=rep("S",each=length(clim))
...
valori_irr_sorghum=rep("S",each=length(clim))
...
valori_irr_soybean=rep("S",each=length(clim))
...
valori_irr_sunflower=rep("S",each=length(clim))
...
valori_irr_potato=rep("D",each=length(clim))
...
valori_irr_tomato=rep("D",each=length(clim))
...
valori_irr_citrus=rep("D",each=length(clim))
...
valori_irr_date_palm=rep("D",each=length(clim))
...
valori_irr_wine_grapes=rep("D",each=length(clim))
...
valori_irr_olives=rep("D",each=length(clim))
...
valori_irr_cotton=rep("S",each=length(clim))
...
valori_irr_alfalfa=rep("S",each=length(clim))
...
valori_irr_pastures=rep("S",each=length(clim))
...
valori_irr_perennial=rep("D",each=length(clim))
...
valori_irr_temporary=rep("S",each=length(clim))
...
dir.create("/data/Input_data/crop_data", recursive = TRUE, showWarnings = FALSE)

# Write crop table to CSV
write.csv(crop_table, file="/data/Input_data/crop_data/crop_table.csv", row.names = F)
