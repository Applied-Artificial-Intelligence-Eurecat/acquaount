...
json_data <- fromJSON("/data/modelconfig.input.json")
...
# Sesto d'impianto
plant_spacing=as.numeric(json_data$plant_spacing) #distance between each plant (m)
line_spacing=as.numeric(json_data$line_spacing) #distance between lines (m)
...
date_past <- as.character(unlist(read.csv(paste(in_dir,"climate_data/date.csv",sep=""))))
...
date_fut <- as.character(unlist(read.csv(paste(in_dir,"climate_data/date_fut.csv",sep=""))))
...
sensor_spacing=json_data$sensor_spacing # spacing between each point of measurement (m)

# Try to read previous run data, if it doesn't exist use only current sensor data
extremes_previous_run <- tryCatch({
  as.numeric(unlist(read.table(paste(in_dir,json_data$extremes_previous_run,sep=""),sep="",header = FALSE)))
}, error = function(e) {
  c(0, 0) # Return default values that won't affect max/min calculations
})

#Identify the maximum value of Soil Moisture ----------
moisture_sensor_raw <- moisture_sensor_raw/100
soil_SAT_sensor=max(c(max(moisture_sensor_raw),extremes_previous_run[1])) #total available water content at saturation BUT RECORDED BY SENSOR!!!
soil_WP_sensor=min(c(min(moisture_sensor_raw),extremes_previous_run[2]))
...
dataset_model <- data_aggregation_function(moisture_sensor,date_past,hh_past,temp_past,precipitation_past,sw_rad_past,RH_past,wind_past,irr_given_vol_original,
...
#moisture_sensor=moisture_sensor*0.5
...
variables_st=list("Date","Hour","precipitation","P_eff","ponding","DP","moisture_LOW","moisture_mean","ET0","Deficit meteo","SAT_daily","theta_fc","TAW","RAW","kc","kc_correction","ETc","ks","Dr","ETa","Irr_req","Deficit final","Irr_vol","Irr_deficit","Irr_vol_percolated","moisture_new")
  ...
  for (crop in crop_list){
    ...
    kc_correction=0
     ...
     for(a in 1:length(l)){ 
          ...
          Area_irr <- irrigated_area_function(irr_method,d_rate,Ks,moisture_sensor,a,Pb,P_sand,P_silt,P_clay,Area_crop,ET0,n_drip_per_line,w_length)
          print(Area_irr)
          
          
          #Determination of irrigation given converted from m3 to mm -----------
          irr_given_mm <- if(length(irr_given_vol) == 0 || is.null(irr_given_vol)) {
            0
          } else {
            irr_given_vol[a]/Area_irr * 1000
          }
        ...
        moisture_mean <- moisture_function(daily_rooting_depth,sensor_level,level,a,moisture_sensor)
        ...
        # Check input lengths before calculating prec_eff
        if(length(daily_rooting_depth) == 0) stop("daily_rooting_depth has length 0")
        if(length(soil_SAT) == 0) stop("soil_SAT has length 0")
        if(length(moisture_mean) == 0) stop("moisture_mean has length 0")
        if(length(ET0) == 0) stop("ET0 has length 0") 
        if(length(precipitation) == 0) stop("precipitation has length 0")
        if(length(irr_given_mm) == 0) stop("irr_given_mm has length 0")
        ...
        # Check input lengths before calculating deficit_meteo
        if(length(soil_SAT) == 0) stop("soil_SAT has length 0")
        if(length(moisture_mean) == 0) stop("moisture_mean has length 0") 
        if(length(daily_rooting_depth) == 0) stop("daily_rooting_depth has length 0")
        if(length(prec_eff) == 0) stop("prec_eff has length 0")
        ...
        # calculate eventual deep percolation
          ...
          if(length(soil_SAT) == 0) stop("soil_SAT has length 0")
          if(length(theta_fc) == 0) stop("theta_fc has length 0")
          if(length(daily_rooting_depth) == 0) stop("daily_rooting_depth has length 0")
...
write.table(c(soil_SAT_sensor,soil_WP_sensor),paste(in_dir,"soil_moisture_data/SM_extremes_previous_run.csv",sep=""),row.names = FALSE,col.names = FALSE)
